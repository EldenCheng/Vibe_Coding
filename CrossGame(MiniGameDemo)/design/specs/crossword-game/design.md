...
1. [软件架构总览](#1-软件架构总览)
   - [1.1 整体架构](#11-整体架构)
   - [1.2 整体架构图](#12-整体架构图)
   - [1.3 页面组织方式](#13-页面组织方式)
2. [模块划分与职责](#2-模块划分与职责)
   - [2.1 WordLoader（单词加载器）](#21-wordloader单词加载器)
   - [2.2 PuzzleGenerator（填字生成器）](#22-puzzlegenerator填字生成器离线脚本)
   - [2.3 DataSourceConfig（配置读取器）](#23-datasourceconfig配置读取器)
   - [2.4 StartPage（启动页面）](#24-startpage启动页面)
   - [2.5 PuzzleBoard（棋盘组件）](#25-puzzleboard棋盘组件)
   - [2.6 GameController（游戏控制器）](#26-gamecontroller游戏控制器)
   - [2.7 ResultPage（结果页面）](#27-resultpage结果页面)
   - [2.9 LetterSelectionPanel（字母选择面板）](#29-letterselectionpanel字母选择面板)
   - [2.10 Timer（计时器模块）](#210-timer计时器模块)
   - [2.11 UserManager（用户管理器）](#211-usermanager用户管理器)
   - [2.12 WrongWordModal（错词弹窗）](#212-wrongwordmodal错词弹窗)
   - [2.13 UserPage（用户页面）](#213-userpage用户页面)
3. [文件结构说明](#3-文件结构说明)
4. [模块间数据流](#4-模块间数据流)
   - [4.1 完整数据流图](#41-完整数据流图)
   - [4.2 StartPage → GamePage 参数传递](#42-startpage--gamepage-参数传递)
   - [4.3 GamePage → ResultPage 参数传递](#43-gamepage--resultpage-参数传递)
5. [关键算法说明](#5-关键算法说明)
   - [5.1 填字棋盘交叉放置算法](#51-填字棋盘交叉放置算法)
   - [5.2 随机抽取不重复关卡序列](#52-随机抽取不重复关卡序列)
   - [5.3 答案比对（不区分大小写）](#53-答案比对不区分大小写)
   - [5.4 平行相邻约束算法](#54-平行相邻约束算法)
   - [5.5 字母面板算法](#55-字母面板算法)
   - [5.6 跨关卡单词去重（生成器）](#56-跨关卡单词去重生成器)
   - [5.7 计时器算法（高级模式专用）](#56-计时器算法高级模式专用)
   - [5.8 初级模式自动填满约束（生成器）](#57-初级模式自动填满约束生成器)
6. [数据结构与 JSON 模式](#6-数据结构与-json-模式)
   - [6.1 PuzzleSet 记录](#61-puzzleset-记录单个关卡-json-结构)
   - [6.2 data-sources.json 配置文件模式](#62-data-sourcesjson-配置文件模式)
   - [6.3 游戏状态对象](#63-游戏状态对象运行时-sessionstorage)
7. [UI 组件设计](#7-ui-组件设计)
   - [7.1 StartPage 布局](#71-startpage-布局)
   - [7.2 GamePage 布局](#72-gamepage-布局)
   - [7.3 ResultPage 布局](#73-resultpage-布局)
8. [状态管理](#8-状态管理)
   - [8.1 状态存储层次](#81-状态存储层次)
   - [8.2 GameController 状态机](#82-gamecontroller-状态机)
   - [8.3 ErrorCount 生命周期](#83-errorcount-生命周期)
   - [8.4 关卡序列管理](#84-关卡序列管理localstorage-进度)
9. [错误处理策略](#10-错误处理策略)
10. [测试策略](#11-测试策略)
    - [10.1 测试工具选择](#111-测试工具选择)
    - [10.2 属性化测试配置要求](#112-属性化测试配置要求)
    - [10.3 单元测试重点](#113-单元测试重点例子测试)
    - [10.4 不适合属性化测试的内容](#114-不适合属性化测试的内容)
11. [技术选型说明](#12-技术选型说明)
12. [从零搭建环境指南](#13-从零搭建环境指南)
    - [12.1 前置要求](#131-前置要求)
    - [12.2 获取项目](#132-获取项目)
    - [12.3 生成关卡数据](#133-生成关卡数据首次必须)
    - [12.4 在浏览器中启动游戏](#134-在浏览器中启动游戏)
    - [12.5 用户系统说明](#135-用户系统说明)
    - [12.6 项目文件结构速览](#136-项目文件结构速览)
    - [12.7 常见问题](#137-常见问题)
...
│
├── css/
│   ├── common.css                         # 公共样式（统一背景色、字体、按钮、桌面端 body 外壳 @media）
...
通过 CSS-only 方案实现，无需修改 HTML。`html` 承载粉底白圆点花纹背景，`body` 自身担任游戏机外壳：

```
  ┌───────────────────────────────────────────┐
  │                                           │  ← html 背景：粉底白圆点花纹
  │  ┌─────────────────────────────────────┐  │
  │  │ ╔═════════════════════════════════╗ │  │  ← body：白边 20px + 圆角 70px
  │  │ ║ ┌───────────────────────────┐ ║ │  │      粉色渐变机身 + 立体阴影
  │  │ ║ │  ┌────┬────┬────┬────┐   │ ║ │  │
  │  │ ║ │  │    │    │    │    │   │ ║ │  │  ← .container
  │  │ ║ │  │    │    │    │    │   │ ║ │  │     内边框 6px #FFB7CE
  ...
  530px × min-height 880px
  body 边缘：白色 20px + 圆角 70px + 粉色立体阴影
  html 背景：粉底白圆点花纹（两圈交错的 radial-gradient）
```

- 外壳高度 `min-height: 880px`，内容自动撑开
- `.container` 铺满外壳内部，获得粉色内边框（6px）和圆角（42px）
- `start.css` 在 ≥800px 时重置 body 高度以防止 height: 100vh 冲突
- 不再使用 HTML 包裹层，手机端零影响
...
本指南面向**新的开发者**，说明如何从克隆项目开始，到在浏览器中成功运行游戏并在本地重新生成关卡数据。

### 13.1 前置要求

| 软件 | 版本要求 | 用途 |
|------|---------|------|
| **Node.js** | ≥ 18.0.0 | 运行离线预生成脚本（生成 PuzzleSet JSON） |
| **现代浏览器** | Chrome / Edge / Firefox 最新版 | 运行游戏（H5 静态页面） |
| **文本编辑器** | 任意（推荐 VS Code） | 修改代码 |

> **注意**：本项目是纯 H5 应用，**不需要** Webpack/Vite 等构建工具。离线脚本仅用于关卡数据预生成，运行后即可关闭。
...
# 克隆仓库（或通过其他方式下载源码）
git clone <仓库地址>
cd CrossGame(MiniGameDemo)
```

### 13.3 生成关卡数据（首次必须）

项目中的 `data/小学-填字游戏.json` 是预先生成的关卡数据。如果需要重新生成（例如修改了词表或生成算法），执行以下步骤：

```bash
# 1. 进入脚本目录
cd scripts

# 2. 运行生成入口脚本（使用 Node.js，无需安装依赖）
node generate.js
```

成功运行后，`data/小学-填字游戏.json` 将被重新生成，包含约 111 组 PuzzleSet（覆盖词表中 ~99% 的单词）。

### 13.4 在浏览器中启动游戏

项目是静态 HTML 页面，无需构建或启动开发服务器：

```bash
# 方法一：直接双击文件
# 在文件管理器中打开项目根目录，双击 start.html

# 方法二：使用 VS Code Live Server
# 1. 安装 Live Server 扩展（Ritwick Dey）
# 2. 右键 start.html → "Open with Live Server"

# 方法三：使用 Python（如果已安装）
cd CrossGame(MiniGameDemo)
py -m http.server 8080
# 然后访问 http://localhost:8080/start.html
```

> **注意**：由于浏览器安全策略，`fetch()` 请求不能从 `file://` 协议加载本地 JSON 文件，因此**必须使用 HTTP 服务器**启动（方法二或三），不能直接双击 `start.html`。
...
1. 点击 **START** 按钮
2. 如果浏览器中已有 7 天内登录过的用户 → 自动进入用户中心
3. 如果有多个用户 → 弹窗让您选择
4. 如果是首次使用 → 输入昵称创建新用户
5. 进入用户中心 → 点击 **开始游戏** 进入填字游戏

所有用户数据存储在浏览器 `localStorage` 中，清除浏览器缓存会丢失数据。

### 13.6 项目文件结构速览

| 目录/文件 | 说明 |
|----------|------|
| `start.html` | 游戏启动页（选范围 + 难度 + 昵称输入） |
| `user.html` | 用户中心（进度统计、错词表、继续游戏） |
| `game.html` | 游戏主页面（填字棋盘 + 字母面板） |
| `result.html` | 游戏结果页（胜利/失败动画） |
| `js/` | 浏览器运行时 JS 模块 |
| `scripts/` | Node.js 离线生成脚本（不部署到浏览器） |
| `css/` | 样式文件 |
| `data/` | 预生成的关卡数据 JSON 和配置文件 |
| `assets/` | 图片资源（Logo、松鼠吉祥物） |

### 13.7 常见问题

**Q：页面打开后下拉菜单显示"加载失败"？**
A：必须通过 HTTP 服务器访问（见 13.4），不能直接双击 HTML 文件。

**Q：点击 START 后没有反应？**
A：检查 `data/data-sources.json` 是否存在，以及 `data/小学-填字游戏.json` 文件路径是否正确。

**Q：如何更换单词库？**
A：编辑 `data/data-sources.json` 添加新的条目，并将对应的 PuzzleSet JSON 文件放入 `data/` 目录。

**Q：如何重置所有用户数据？**
A：在浏览器开发者工具中执行 `localStorage.clear()`，或在应用设置中清除该网站的存储数据。

**Q：如何新增单词？**
A：编辑 `广州科教版小学英语三至五年级单词表.json` 添加词条，然后重新运行 `node scripts/generate.js` 重新生成关卡数据。

**Q：游戏无法正常启动/出现 Bug？**
A：查看浏览器控制台（F12 → Console）的报错信息，根据错误提示排查问题。也可查看 `版本变更.md` 确认当前版本状态。
