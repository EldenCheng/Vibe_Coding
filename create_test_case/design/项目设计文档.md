...
```bash
py scripts\main.py --input <输入文件路径> [--prompts <可选提示词1,可选提示词2>] [--temp-prompt "临时提示词内容"]
```
...
```bash
# 基本用法：只使用基础分析提示词
py scripts\main.py --input Flowchart.pdf

# 使用可选提示词
py scripts\main.py --input Flowchart.pdf --prompts Flowchart

# 使用多个可选提示词
py scripts\main.py --input Flowchart.pdf --prompts Flowchart,AnotherPrompt

# 使用临时提示词
py scripts\main.py --input Flowchart.pdf --temp-prompt "请特别关注状态机的转换逻辑"

# 完整用法
py scripts\main.py --input Flowchart.pdf --prompts Flowchart --temp-prompt "特殊分析要求"

# 查看帮助
py scripts\main.py -h
```
...
```bash
py scripts\analysis_processor.py --images-dir output/images --output-dir output/analysis [--prompts Flowchart] [--temp-prompt "特殊分析要求"]
```
...
| `--project-root` | 项目根目录路径         | 否（默认为脚本所在目录的上一级） |
...
```bash
py scripts\test_case_generator.py --analysis-dir output/analysis --output output/test_cases.json
```
...
```bash
py scripts\test_matrix_generator.py --input-json output/test_cases.json --output-dir output
```
