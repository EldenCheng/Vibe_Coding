...
* **执行动作 (Action)**：**矩形** $\rightarrow$ 表示系统执行的具体指令, 物理动作或者播放声音。
...
* 从每个块的起点开始，沿着箭头追踪。
* 分析所有**菱形**的 `yes/no` 分支。严格区分**判定条件**（菱形内部的文字）与**跳转指令**（椭圆或者矩形内部的文字）
...
2. **逻辑判定树**：为每个流程块详细列出判定条件、分支结果及循环逻辑外, 还需要在对应分支中查找与列出属于这个分支的所有动作序列, 执行的动作，并注明同步发生的动作。
3. **动作序列**：需要详细列出所有动作序列, 执行的动作，并注明同步发生的动作。
4. **中断机制**：列出所有被识别为“全局中断”的流程块及其触发条件。
5. **输入清单**：所有输入, 比如Watering button等。
6. **状态判断清单**：所有状态判断, 比如30 sec timeout, Water Count = 3等。
7. **资源清单**：列出所有系统执行的具体指令, 物理动作或者播放声音 , 比如STAJING, Low volume set等。
